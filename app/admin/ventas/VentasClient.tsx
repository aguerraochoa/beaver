              ...
              {expandedItems.has(venta.venta_id) && (
                <div className="px-4 pb-4 border-t border-gray-100 dark:border-slate-700/50 bg-gray-50/50 dark:bg-slate-800/50">
                  <div className="grid grid-cols-2 gap-3 pt-3">
                    <div>
                      <p className="text-xs text-slate-500 dark:text-slate-400 mb-1">Vendedor</p>
                  ...
                  {venta.estado === 'pendiente' && (
                    <div className="flex gap-2 pt-4 mt-2 border-t border-gray-200 dark:border-slate-700">
                      <button
                        onClick={(e) => { e.stopPropagation(); handleApprove(venta.venta_id) }}
                        disabled={processing === venta.venta_id}
                        className="flex-1 flex items-center justify-center gap-2 p-2 bg-green-600 hover:bg-green-700 text-white rounded-lg shadow-sm disabled:opacity-50 transition-colors font-medium text-sm"
                      >
                      ...
                      <button
                        onClick={(e) => { e.stopPropagation(); handleReject(venta.venta_id) }}
                        disabled={processing === venta.venta_id}
                        className="flex-1 flex items-center justify-center gap-2 p-2 bg-white hover:bg-red-50 text-red-600 border border-red-200 hover:border-red-300 rounded-lg shadow-sm disabled:opacity-50 transition-colors font-medium text-sm"
                      >
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  {venta.estado === 'aprobada' && venta.aprobado_por_usuario && (
                    <div className="pt-2 mt-2 border-t border-gray-200 dark:border-slate-700">
                      <p className="text-xs text-slate-500 dark:text-slate-400">
                        Aprobada por <span className="font-medium text-slate-700 dark:text-slate-300">{venta.aprobado_por_usuario.nombre}</span>
                      </p>
                    </div>
                  )}
                </div>
